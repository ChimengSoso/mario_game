...
function Map:init()
  ...
  self.music = love.audio.newSource('sounds/music.wav', 'static') -- from freesound.org
...
-- sets a tile at given x-y coordinate to an integer value
...
function Map:tileAt(x, y)
  local newx = math.floor(x / self.tileWidth) + 1
  local newy = math.floor(y / self.tileHeight) + 1
  return {['id'] = self:getTile(newx, newy),
          ['x'] = newx,
          ['y'] = newy}
end

-- return an integer value for the tile at a given x-y corrdinate
